...
test("renders with correct layout classes", () => {
  // Test with messages to get the ScrollArea layout
  (useChat as any).mockReturnValue({
    ...mockUseChat,
...
test("renders centered layout when no messages", () => {
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [],
  });
  ...
  const messageListContainer = screen.getByTestId("message-list").closest(".flex-1");
  expect(messageListContainer?.className).toContain("flex");
  expect(messageListContainer?.className).toContain("items-center");
  expect(messageListContainer?.className).toContain("justify-center");
});